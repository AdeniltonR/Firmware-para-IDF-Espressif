...
#define I2C_MASTER_SCL_IO           7    // Pino SCL
...
static float yaw = 0.0f;  // Ângulo Z acumulado

static uint64_t last_time_us = 0; // Para cálculo do delta time
...
// Função para calcular Yaw integrando o giroscópio Z
static void update_yaw(float gyro_z_dps)
{
    ...
    float dt = (now - last_time_us) / 1000000.0f; // segundos
    last_time_us = now;

    yaw += gyro_z_dps * dt; // integra velocidade angular

    // Mantém yaw entre -180 e 180
...
static void imu_task(void *arg)
{
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "ROLL: %.2f°  PITCH: %.2f°  YAW: %.2f°", angle.roll, angle.pitch, yaw);
