...
//---máscara de bit para identificar o estado de conexão---
#define WIFI_CONNECTED_BIT BIT0  // ajuste se necessário
...
void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    //---se o evento for de desconexão (WIFI_EVENT_STA_DISCONNECTED), tenta reconectar---
        ...
        if (s_retry_num < EXAMPLE_ESP_MAXIMUM_RETRY) {
            ...
            ESP_LOGW(TAG, "🚫 Número máximo de tentativas excedido. Reiniciando o ESP32...");
...
void wifi_init_sta(void) {
    ...
    if (load_wifi_credentials(ssid, sizeof(ssid), password, sizeof(password)) != ESP_OK) {
        ESP_LOGE(TAG, "❌ Falha ao carregar credenciais da NVS. Usando valores padrão.");
    ...
    if (strlen(ssid) == 0 || strlen(password) == 0) {
        ESP_LOGW(TAG, "⚠️  Credenciais Wi-Fi inválidas! SSID ou senha estão vazios.");
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(TAG, "📶 Conectado ao AP SSID:%s senha:%s", ssid, password);
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI(TAG, "❌ Falha ao conectar ao SSID:%s, senha:%s", ssid, password);
    } else {
 ...
 * @return true se o bit WIFI_CONNECTED_BIT estiver ativo no Event Group, false caso contrário ou se o Event Group não estiver inicializado
 * 
 * @note Esta função:
 * 1. Faz uma verificação de segurança do ponteiro s_wifi_event_group
 * 2. Loga erro se o Event Group não estiver inicializado (nível ERROR)
 * 3. Retorna o estado do bit de conexão WiFi de forma não-bloqueante
 * 
 * @warning Requer que s_wifi_event_group tenha sido inicializado previamente
 *          com xEventGroupCreate() durante a configuração do WiFi
 */
bool wifi_is_connected(void) {
    if (s_wifi_event_group == NULL) {
        ESP_LOGE(TAG, "🔄 Event Group não inicializado!");
        return false;
    }
